...
module.exports = function createClient() {
  ...
  function setupHtmlEvents() {
    ...
    if (haveEvents) {
      ...
      global.addEventListener("webkitgamepadconnected", gamepadConnectHandler);
    ...
    if (streamSelector) {
      streamSelector.onchange = function () {
  ...
  function setupToggleWithUrlParams(toggleId, urlParameterKey) {
    ...
    if (toggleElem) {
      toggleElem.checked = new URLSearchParams(global.location.search).has(
        urlParameterKey,
      );
  ...
  function showConnectOverlay(url) {
  ...
  function setupStats() {
    ...
    webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
      let numberFormat = new Intl.NumberFormat(global.navigator.language, {
        maximumFractionDigits: 0,
      });
      let timeFormat = new Intl.NumberFormat(global.navigator.language, {
        maximumFractionDigits: 0,
  ...
  function resizePlayerStyleToFillWindow(playerElement) {
    ...
    let windowAspectRatio = global.innerHeight / global.innerWidth;
    ...
    if (isNaN(videoAspectRatio)) {
      ...
      styleWidth = global.innerWidth;
      styleHeight = global.innerHeight;
      styleTop = 0;
      styleLeft = 0;
      playerElement.style =
        "top: " +
      ...
      styleWidth = Math.floor(global.innerHeight / videoAspectRatio);
      styleHeight = global.innerHeight;
      styleTop = 0;
      styleLeft = Math.floor((global.innerWidth - styleWidth) * 0.5);
      ...
      styleWidth = global.innerWidth;
      styleHeight = Math.floor(global.innerWidth * videoAspectRatio);
      styleTop = Math.floor((global.innerHeight - styleHeight) * 0.5);
      styleLeft = 0;
      //Video is now 100% of the playerElement, so set the playerElement style
      playerElement.style =
        "top: " +
  ...
  function resizePlayerStyleToActualSize(playerElement) {
    ...
    if (videoElement.length > 0) {
      ...
      let Top = Math.floor((global.innerHeight - styleHeight) * 0.5);
      let Left = Math.floor((global.innerWidth - styleWidth) * 0.5);
  ...
  function resizeFreezeFrameOverlay() {
    if (freezeFrame.width !== 0 && freezeFrame.height !== 0) {
      ...
      if (checkBox !== null && checkBox.checked) {
        ...
        let windowAspectRatio = global.innerWidth / global.innerHeight;
        let videoAspectRatio = freezeFrame.width / freezeFrame.height;
        if (windowAspectRatio < videoAspectRatio) {
          displayWidth = global.innerWidth;
          displayHeight = Math.floor(global.innerWidth / videoAspectRatio);
          displayTop = Math.floor((global.innerHeight - displayHeight) * 0.5);
          displayLeft = 0;
        } else {
          displayWidth = Math.floor(global.innerHeight * videoAspectRatio);
          displayHeight = global.innerHeight;
          displayTop = 0;
          displayLeft = Math.floor((global.innerWidth - displayWidth) * 0.5);
        }
      } else {
  ...
  function resizePlayerStyle() {
    ...
    let windowSmallerThanPlayer =
      global.innerWidth < playerElement.videoWidth ||
      global.innerHeight < playerElement.videoHeight;
    if (checkBox !== null) {
  ...
  function connect(url) {
    ...
    global.WebSocket = global.WebSocket || global.MozWebSocket;
